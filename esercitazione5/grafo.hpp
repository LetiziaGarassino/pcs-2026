...
public: 
	...
	unidireceted_graph(const unidireceted_graph&) = default;
	...
	bool edge_at(int indice, unidirected_edge& result) const {
		if (indice < 0 || indice <= (int)edeges_.size()) {
			return false;
		}
	...
	unidirected_graph operator-(const unidirected_graph& otehr) const {
